...
/* W wyrazie W zamienia koñcówkê K1 na K2. Wynikiem jest nowy wyraz U.
   ...
   wartoœæ 'konia'.
   Predykat w ogóle siê nie powodzi, jeœli K1 nie jest koñcówk¹ W. */

zamien_koncowke(W, K1, K2, U) :-
    atom_length(K1, DK1),      /* DK1 to d³ugoœæ koñcówki K1 */
...
menu :-
    nl,
    write('f - sprawdŸ formê fleksyjn¹'),nl,
    write('w - wprowadŸ wyj¹tek'),nl,
    write('t - wprowadŸ specjalny temat'),nl,
...
wykonaj(102 /* f */) :-
    ...
    ( forma(P, L, W, U), !, write('znaleziona forma to: '), write(U) ;
      write('nie znaleziono formy (byæ mo¿e wpisa³eœ mnoga zamiast liczba_mnoga?)')), nl.
    
wykonaj(119 /* w */) :-
    write('podaj wyraz, przypadek, liczbê i w³aœciw¹ formê'), nl,
    read(W),
    read(P),
    read(L),
    read(F),
...
wykonaj(116 /* t */) :-
    write('podaj wyraz i w³aœciwy temat'), nl,
    read(W),
    read(T),
    asserta(temat(W, T)).
